...
public class Search5 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //searchs5.jspからデータを受け取る
            //各項目 name,age,birthdayデータをgetParameterで受け取る
            ...
            try{
                ...
                //部分一致表示　SELECT name,age FROM profiles WHERE name = ? OR age = ?…
                //db_st = db_con.prepareStatement("SELECT * FROM profiles WHERE name = ? OR age = ? OR birthday = ?");
                //完全一致表示　SELECT name,age FROM profiles WHERE name = ? OR age = ?…
                db_st = db_con.prepareStatement("SELECT name, age, birthday FROM profiles WHERE name = ? AND age = ? AND birthday = ?");
                //nameを指定
                ...
                out.print("<h2>対象データを表示します</h2><br>");
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>複合検索 結果</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("</body>");
            out.println("</html>");
